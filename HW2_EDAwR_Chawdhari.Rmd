...
There are no NA's in the countyMI data set while there are two NA's for the Income and IncomeError Field each.
Structure wise there is an extra Peninsula column in the CountyMI data set,as a result Region Colum does not have any categories
...
ggplot(data=county_df, aes(y = county_df$Income,x = "count")) + 
  geom_violin(fill="Light Blue") +
    labs(title="Violin Chart for Median Household Income for US" , x="Income")

quantile(county_df$Income)


```
  1. 50 % of the people have median household income from 37500 to 51000 aprx
  2.There are many outliers on the higher side i.e people earning more than 75000
  3.The histogram is skewed to the right.
  4.There is an income inequality,this can be seen from the box plot,range for 4 th quartile  is way greater than combined range for     first 3 quartiles i.e 75% of the people had a household income that falls between a short range while household income for rest     25% is spread out,
...
ggplot(data=county_df, aes(county_df$TotalPop)) + 
  geom_histogram()+
  labs(title="Population by Counties in US",x="Income",y="Count of Counties")
...
4.There is more income inequality in Lower Peninsula as compared to Upper Peninsula,The Income for Upper Peninsula seems to uniformly distributed as compared to LP.
...
ggplot(data =  county_MI, aes(x = county_MI$peninsula,y = county_MI$Poverty)) +
  geom_boxplot(colour="black",fill="light blue",outlier.colour = "red")+
  labs(title = "Michigan Population by Peninsula",x="Peninsula" , y="Poverty" )

#Boxplot for Income
ggplot(data =  county_MI, aes(x = county_MI$peninsula,y = county_MI$Income)) +
  geom_boxplot(colour="black",fill="light blue",outlier.colour = "red")+
  labs(title = "Michigan Population by Peninsula",x="Peninsula" , y="Income" )
...
mdata <- melt(data = county_MI, id = 'peninsula', measure = c ('Hispanic','White','Black','Native','Asian'))
 ...
 mdata <- melt(data = county_MI, id = 'peninsula', measure = c ('Professional','Service','Office','Production'))
 ggplot(mdata) + geom_boxplot(aes(x=peninsula, y=value, color=variable)) 
 
  #Grouped by Type of Employment 
 mdata <- melt(data = county_MI, id = 'peninsula', measure = c ('PrivateWork','PublicWork','SelfEmployed','FamilyWork'))
 ggplot(mdata) + geom_boxplot(aes(x=peninsula, y=value, color=variable)) 
...
ggplot(data=county_df, aes(county_df$TotalPop)) + 
  geom_histogram(colour ="Red", fill ="light blue") + 
  labs(title="Distribution of population for US counties" ,x="TotalPop",y="Number of counties") + 
  facet_grid(. ~ Region)

ggplot(data=county_df, aes(county_df$Poverty)) + 
  geom_histogram(colour ="Red", fill ="light blue") + 
  labs(title="Distribution of population for US counties" ,x="Poverty",y="Number of counties") + 
  facet_grid(. ~ Region)

ggplot(data=county_df, aes(county_df$MeanCommute)) + 
  geom_histogram(colour ="Red", fill ="light blue") + 
  labs(title="Distribution of population for US counties" ,x="MeanCommute",y="Number of counties") + 
  facet_grid(. ~ Region)


ggplot(data=county_df, aes(county_df$IncomePerCap)) + 
  geom_histogram(colour ="Red", fill ="light blue") + 
  labs(title="Distribution of population for US counties" ,x="IncomePerCap",y="Number of counties") + 
  facet_grid(. ~ Region)
...
ggplot(county_MI,aes(x= log10(county_MI$Employed)  ,y =log10(county_MI$Poverty) )) + 
  geom_point() + 
  labs(title = "Number of Counties By Region", x="Employed" , y="Poverty") +
